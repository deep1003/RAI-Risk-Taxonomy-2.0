...
function openCard(l4Id) {
  ...
  ui.dialogContent.innerHTML = `<div class="dialog-body" style="--card-accent:${domainColor}">
    ...
    ${(card.human_reviews || []).length ? `<section class="dialog-section"><h3>휴먼 리뷰</h3>${card.human_reviews.map((h) => `<div class="human-review-block"><p class="definition-ko"><strong>검토의견</strong> ${escapeHtml(h.comment)}</p><p class="definition-ko"><strong>반영 결과</strong> ${escapeHtml(h.result)}</p></div>`).join("")}</section>` : ""}
    ${(card.references || []).length ? `<section class="dialog-section"><h3>References</h3>${card.references.map((r) => `<p class="definition-ko"><a href="${escapeHtml(r.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(r.label)}</a></p>`).join("")}</section>` : ""}
  </div>`;
  ui.dialog.showModal();
}
