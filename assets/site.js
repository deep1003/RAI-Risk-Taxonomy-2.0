...
function openCard(l4Id) {
  ...
  ui.dialogContent.innerHTML = `<div class="dialog-body" style="--card-accent:${domainColor}">
    ...
    ${(card.references || []).length ? `<section class="dialog-section"><h3>Evidence reference</h3>${card.references.map((r) => `<div class="evidence-reference-block"><p class="definition-ko"><a href="${escapeAttribute(r.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(r.title || r.label)} ↗</a></p>${r.authors || r.year || r.type ? `<p class="definition-ko">${[r.authors, r.year, r.type].filter(Boolean).map(escapeHtml).join(" · ")}</p>` : ""}${r.quote ? `<blockquote class="reference-quote">${escapeHtml(r.quote)}</blockquote>` : ""}${r.quote_location ? `<p class="definition-ko"><small>${escapeHtml(r.quote_location)}</small></p>` : ""}</div>`).join("")}</section>` : ""}
  </div>`;
  ui.dialog.showModal();
}
