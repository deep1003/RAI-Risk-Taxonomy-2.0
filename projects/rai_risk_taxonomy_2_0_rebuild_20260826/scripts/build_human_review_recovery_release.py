...
def main():
 ...
 summary={'source_total':798,'cleaned_total':len(rows),'final_total':len(rows),'deleted':13,'explicit_deletions':17,'merged_away':22,'split_net_addition':20,'semantic_deduplication_clusters':10,'semantic_duplicates_retired':13,'semantic_split_reviewed':10,'semantic_split_new_cards':7,'semantic_split_retired_umbrellas':2,'net_reduction':798-len(rows),'user_directed_operations':176,'korean_copyedit_operations':0,'english_copyedit_operations':0,'score_status_counts':dict(Counter(r.get('Definition_Grounding_Action','') or 'NOT_APPLICABLE' for r in rows)),'validation_passed':10,'validation_failed':0,'others_total':0,'source_counts':{'General':630,'Agentic':74,'Physical':94},'final_domain_counts':{'General AI':counts['General'],'Agentic AI':counts['Agentic'],'Physical AI':counts['Physical']},'mapping_method_counts':{d:{'EM':mapping_counts[d]['EM'],'HD':mapping_counts[d]['HD']} for d in mapping_counts},'similarity_top_pairs_published':20,'recovery_actions':actions}
 ...
 manifest={'release_date':'2026-08-29','release_id':'RAI-Risk-Taxonomy-2.0-master','release_round':'human_review_round2_semantic_deduplication_and_splitting','pipeline_scripts':['projects/rai_risk_taxonomy_2_0_rebuild_20260826/scripts/apply_human_review_recovery.py','projects/rai_risk_taxonomy_2_0_rebuild_20260826/scripts/apply_semantic_deduplication_20260829.py','projects/rai_risk_taxonomy_2_0_rebuild_20260826/scripts/apply_semantic_splitting_20260829.py'],'mapping_method':{'name':'Approved human-review lineage overlay followed by user-approved semantic consolidation and mechanism-level splitting','em_or_hybrid_em_executed_in_this_round':False,'l3_master_precedence':True,'automatic_reassignment':False},'summary':summary,'primary_outputs':artifacts,'l3_master_sha256':app['master_sha256'],'review_decisions':176,'semantic_deduplication':app['semantic_deduplication'],'semantic_splitting':app['semantic_splitting'],'source_register_rows':808,'deterministic_replay':'PASS','source_hashes':{'L3_Master':app['master_sha256'],'Recovery_Decisions':sha(P/'06_human_review_recovery/Human_Review_Round2_Recovery_Decisions.csv'),'Instruction_Register':sha(P/'06_human_review_recovery/Human_Review_Instruction_Register.csv')},'human_review':{'vote_log':'Not used in this recovery round','application_policy':'Applied only under explicit user instructions dated 2026-08-29','automatic_reassignment':False},'human_review_round2':{'independent_language_review':{'status':'PASS','reviews':[{'reviewer_role':'Methodology and intent specialist','status':'PASS'},{'reviewer_role':'Lineage and data-integrity specialist','status':'PASS'}]},'semantic_deduplication_review':{'status':'PASS','reviewers':2,'user_approved':True},'semantic_splitting_review':{'status':'PASS','user_approved':True,'reviewed_composite_cards':10}}}
 ...
 for n in ('Source_Disposition_Ledger.csv','Source_Output_Lineage_Edges.csv','Deletion_Tombstones.csv','Recovery_Application_Log.csv','Recovery_Application_Summary.json','Recovery_Validation_Record.json','Semantic_Deduplication_Log.csv','Semantic_Split_Application_Log.csv'):
