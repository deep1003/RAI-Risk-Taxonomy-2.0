...
\section{요약}
본 작업은 초기 의미 유사도 파이프라인이 응급·재난 대응과 같은 적용 맥락을 폭력 리스크로 오분류한 문제를 계기로 RAI Risk Taxonomy 2.0을 원천 데이터부터 다시 구축한 결과이다. 원천 L4 892건을 동결하고, 의미적 매핑 전에 리스크 적격성 심사, 삭제, 통합, 분리, 한영 명칭·정의 재작성, 동료 검토를 수행하였다. 최종 L4는 808건이며 보존식은 \(892-65-20+1=808\)이다.

최종 카드 중 612건(75.7\%)은 기존 L3에 EM으로 배정했고, 196건(24.3\%)은 기존 L3 후보 사이의 경계성, 낮은 확신, 불안정성 또는 한영 불일치 때문에 도메인별 Others와 인간 결정(HD) 대기열로 분리하였다. 최종 분포는 General 599건, Agentic 90건, Physical 119건이다. 원본 L3 46행은 변경하지 않았고, 도메인별 Others 3행만 파생 구조로 추가하였다. 내부 자동 검증 50개는 모두 통과하였다.
...
General AI & 591 & 599 & +8\\ Agentic AI & 83 & 90 & +7\\ Physical AI & 218 & 119 & -99\\\midrule 합계 & 892 & 808 & -84\\\bottomrule\end{tabular}\end{table}
...
명시적 삭제 39건, 비리스크 적용 맥락 8건, 동료 검토 폐기 1건을 삭제 보관본으로 이동하였다. 14개 통합 그룹에서 20건을 대표 카드로 흡수했고, 다중 의미 카드 1건을 두 카드로 분리하여 순증 1건을 만들었다. 원천에서 `new'로 표시된 100건 가운데 비리스크 맥락 8건을 제거하고 87건을 재정의·재발급하였다.
...
이후 825개 카드를 L3 마스터의 명칭·정의와 전수 대조했다. 548건은 이미 AI 기술 주체와 위험 구조가 완결되어 유지했고, 260건은 원래 의미를 보존하면서 한·영 정의에 AI 시스템·알고리즘·에이전트·로봇·학습 기술 또는 모델을 명시하도록 보완했다. 리스크가 아니거나 분류체계 전체 범위 밖인 12건은 삭제 보관하였다. 확정된 L1에 속하지만 정확한 현행 L3가 없는 14건과 후보 간 경계가 모호한 카드는 해당 L1의 Others에 보류하였다.

최종 명칭 중 66건은 관용적 AI 개입 수식어를 제거하거나 표준 용어로 정규화했다. 유지된 808건 모두에 L3 마스터와 ISO, NIST, OECD, UNESCO, UNICEF, WHO, UNODC, WIPO, IMF 또는 국내 AI 기본법의 용어군 근거를 기록했다. 단, 기관 문구를 카드명에 기계적으로 복사하지 않고 해당 L3의 위해·실패·침해 개념과 일치하는 명사구로 검증했다.

813개 수정 카드를 대상으로 같은 L3 안의 한영 의미 유사도 후보 66쌍을 검토했다. 유사도는 후보 생성에만 사용했고, 보호 특성, 무기 유형, 피해 대상 또는 위해 메커니즘이 다르면 61쌍을 유지했다. 별도 의미를 추가하지 않는 하위·포괄 중복 5건만 대표 카드에 연결하여 삭제 보관했다.

\begin{table}[htbp]\centering\caption{고유사도 L4 중복 검토 결과}
\begin{tabular}{lrr}\toprule 판정 & 후보 쌍 & 카드 삭제 수\\\midrule
구별되는 범위로 유지 & 61 & 0\\ 대표성이 낮은 중복 폐기 & 5 & 5\\\midrule 합계 & 66 & 5\\\bottomrule\end{tabular}\end{table}
...
\section{L1 라우팅과 ID}
Instruction Prompt가 명시한 L1 이동 103건과 별도 해결 지시 1건을 반영하였다. 여기에 L1을 먼저 확정하고 그 안에서 L3를 배정하는 교차 도메인 검토 단계를 추가해 30건을 재검토했다. 이 가운데 현행 L3와 정확히 맞지 않는 14건은 다른 L1의 유사한 L3로 밀어 넣지 않고, 검토된 L1의 Others로 보류하였다. 동료 검토로 메커니즘이 변경된 RAI4-1157 한 건은 의미 기준으로 General에 추가 이동하였다. \texttt{facet}과 \texttt{act-type}은 L4 속성으로 유지했지만 의미 매핑 입력에서는 제외했다. 모든 L4 ID는 \texttt{L3\_ID + 순번} 형식으로 다시 발급하였다.
...
기준 키워드를 사용하지 않은 배정선에서는 EM 559건, HD 266건이었다. 키워드 보강과 L3 기반 정의 검토 후 EM은 612건으로 53건 늘었으며, 임금 양극화는 G\_SOC\_ECON, 대규모 사이버범죄 오용은 G\_SYS\_SECADV로 회수했다. 단, HD 196건은 매핑 실패가 아니라 경계 사례를 명시적으로 격리한 결과이며 자동 확정된 L3로 해석해서는 안 된다.
...
자동 검증은 원천 해시, L3 불변성, 수량 보존, ID 유일성, 한영 필드, AI 기술 명시, L3식 정의문 구조, 명칭 용어 근거, 관용적 AI 수식어 제거, 중복 후보 판정과 대표 카드 계보, L3 범위 게이트, Others의 HD 전용성, crosswalk, 3개 대표 키워드, 상위 2개 후보, 5개 CSV 구성을 포함한 50개 항목에서 모두 PASS였다.
...
\section{결론}
재구축은 어휘 유사도가 정상 응급 대응을 폭력으로 오인한 오류를 리스크 적격성 문제로 재정의하였다. 적격성 게이트, L3 불변성, 한영 정의 재작성, 보수적 EM과 HD 격리를 결합함으로써 808개 L4와 완전한 계보를 가진 RAI Risk Taxonomy 2.0 master를 생성하였다.
\end{document}
