...
SOURCE = ROOT / "public/data/releases/v2.10.0"
RELEASE = ROOT / "public/data/releases/v2.11.0"
REPORT = ROOT / "reports/data_quality/copyrights_all_l3_em_v2.11.0"


class CopyrightsAllL3EmV211Tests(unittest.TestCase):
    @classmethod
    ...
    def test_convergence_and_source_scope(self) -> None:
        ...
        self.assertTrue(all(row["from_l3_id"] == "RAI3-G-INT-08" for row in self.audit))
    ...
    def test_only_audited_paths_change(self) -> None:
        ...
        changed = {
            card["l4_id"] for card in self.after
            if card["primary_l3_id"] != before[card["l4_id"]]["primary_l3_id"]
        }
