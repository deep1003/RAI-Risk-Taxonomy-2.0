...
최종 카드 중 613건(75.9\%)은 기존 L3에 EM으로 배정했고, 195건(24.1\%)은 기존 L3 후보 사이의 경계성, 낮은 확신, 불안정성 또는 한영 불일치 때문에 도메인별 Others와 인간 결정(HD) 대기열로 분리하였다. 최종 분포는 General 618건, Agentic 78건, Physical 112건이다. 원본 L3 46행은 변경하지 않았고, 도메인별 Others 3행만 파생 구조로 추가하였다. 내부 자동 검증 47개는 모두 통과하였다.
...
General AI & 591 & 618 & +27\\ Agentic AI & 83 & 78 & -5\\ Physical AI & 218 & 112 & -106\\\midrule 합계 & 892 & 808 & -84\\\bottomrule\end{tabular}\end{table}
...
이후 825개 카드를 L3 마스터의 명칭·정의와 전수 대조했다. 548건은 이미 AI 기술 주체와 위험 구조가 완결되어 유지했고, 260건은 원래 의미를 보존하면서 한·영 정의에 AI 시스템·알고리즘·에이전트·로봇·학습 기술 또는 모델을 명시하도록 보완했다. 현행 L3 중 어느 범위에도 들어가지 않거나 단일 L3로 표현할 수 없는 12건은 삭제 보관하였다. Others는 하나 이상의 현행 L3 범위에는 들어가지만 후보 간 경계가 모호한 경우에만 사용하였다.
...
General AI & 618 & 457 & 161 & 26.1\%\\ Agentic AI & 78 & 65 & 13 & 16.7\%\\ Physical AI & 112 & 91 & 21 & 18.8\%\\\midrule 합계 & 808 & 613 & 195 & 24.1\%\\\bottomrule\end{tabular}\end{table}
...
General AI & 0.7588 & 0.0304 & 0.9854 & 62.3\%\\ Agentic AI & 0.7606 & 0.0346 & 0.9897 & 71.8\%\\ Physical AI & 0.7316 & 0.0251 & 0.9964 & 64.3\%\\\bottomrule\end{tabular}\end{table}
...
기준 키워드를 사용하지 않은 배정선에서는 EM 559건, HD 266건이었다. 키워드 보강과 L3 기반 정의 검토 후 EM은 613건으로 54건 늘었으며, 임금 양극화는 G\_SOC\_ECON, 대규모 사이버범죄 오용은 G\_SYS\_SECADV로 회수했다. 단, HD 195건은 매핑 실패가 아니라 경계 사례를 명시적으로 격리한 결과이며 자동 확정된 L3로 해석해서는 안 된다.
...
자동 검증은 원천 해시, L3 불변성, 수량 보존, ID 유일성, 한영 필드, AI 기술 명시, L3식 정의문 구조, 명칭 용어 근거, 관용적 AI 수식어 제거, 중복 후보 판정과 대표 카드 계보, L3 범위 게이트, Others의 HD 전용성, crosswalk, 3개 대표 키워드, 상위 2개 후보, 5개 CSV 구성을 포함한 47개 항목에서 모두 PASS였다.
...
\section{한계와 후속 검토}
EM은 확률 혼합모형의 완전한 EM이 아니라 L3 anchor를 둔 제약 구면 hard EM이다. 점수 임계값은 내부 운영 기준이며 전문가 라벨로 외부 보정되지 않았다. Others 195건은 삭제 대상이 아니라 인간 결정 대기열이다. 향후 독립 검증은 도메인·L3·점수 구간을 층화한 표본을 사용해 자동 배정의 정확도와 HD 회수율을 평가해야 한다.
