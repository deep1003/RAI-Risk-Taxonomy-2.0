...
Three conclusions follow from Table~
ef{tab:v217-bge} without restating its
...
Table~
ef{tab:v2172-bge-active} shows that the consolidation and guarded
...
superseded release. The procedure is identical to the historical analysis:
the same seed, matched-size permutation design, and perturbation schedule are
applied to the active cards, first with the HOLD queue embedded in its stored
semantic lineage and then with the queue removed before centroid
construction. Because the canonical BGE-M3 encoder is not available in the
environment that produced this rerun, the analysis uses the independent
multilingual-e5 encoder; the two conditions are therefore compared with each
other, not with the BGE-M3 tables above, and the rerun doubles as an
encoder-transfer replication of the sensitivity result.

\begin{table}[ht]
\centering
\caption{HOLD sensitivity at v2.17.2 under the transfer encoder
(multilingual-e5-small, seed 20260721, 5{,}000 permutations, 200 perturbation
repeats per $\sigma$).}
...
Top-1 containment & 64.0\% & 76.9\% \\
Top-2 containment & 79.3\% & 87.9\% \\
Top-3 containment & 85.9\% & 90.6\% \\
Top-5 containment & 92.9\% & 94.9\% \\
Median assignment margin & 0.0032 & 0.0072 \\
Negative-margin share & 36.0\% & 23.1\% \\
Permutation $p$ & 0.0002 & 0.0002 \\
EM iterations to fixed point & 11 & 11 \\
EM agreement with released paths & 22.6\% & 27.9\% \\
Agreement at $\sigma=0.01$ & 91.0\% & 92.9\% \\
Agreement at $\sigma=0.05$ & 53.5\% & 60.7\% \\
\bottomrule
\end{tabular}
\end{table}

The qualitative pattern of the historical analysis replicates exactly under a
different encoder and after two release cycles of consolidation: removing the
...
reading remains conditional, as in the historical analysis: the non-HOLD
subset is cleaner because the policy deliberately concentrates weak-fit and
multi-mechanism cases in the queue, so the comparison validates the
separation signal, not the accuracy of individual non-HOLD assignments.
Machine-readable outputs are stored in
\code{reports/validation/v2\_17\_2\_hold\_sensitivity\_e5/}.
