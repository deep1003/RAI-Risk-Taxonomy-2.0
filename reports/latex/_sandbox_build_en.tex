...
locked Physical cards across 50 semantic L3 nodes. Release v2.15.0 then adds
domain-specific HOLD review paths under General and Agentic AI, moves 626 and
94 review cards respectively, and preserves each original semantic L3 as
lineage metadata. Incremental
...
50-family assignment when HOLD remains embedded (64.7\% top-1 containment;
72.8\% perturbation agreement at $\sigma=0.05$). Re-evaluation of the 991
non-HOLD cards after separation yields 75.8\% top-1 containment and 78.4\%
agreement. A dedicated sensitivity analysis shows that HOLD exclusion improves
EM convergence, released-L3 top-$k$ containment, and perturbation stability,
but this is a conditional subset result rather than a like-for-like accuracy
gain. We specify every evidence filter, classification
...
\subsection{Incremental constrained-EM audits, v2.9.0 to v2.12.0}
...
\subsection{Reliability after excluding the HOLD review paths, v2.15.0}
...
\subsection{HOLD sensitivity analysis}
...
\subsection{v2.17.0-rc: definition revision pass and guarded constrained-EM audit}

Release candidate v2.17.0-rc applies an expert editorial pass to the
non-Physical registry while keeping all 182 Physical cards byte-identical.
The pass rewrites 147 title--definition rewrite candidates identified by the
v2.16.0 remediation audit into mechanism-only bilingual definitions, renames
16 non-descriptive labels with the original label preserved in provenance,
applies 187 mechanical hygiene fixes (hyphenation artifacts, missing final
punctuation, duplicated tokens) to the remaining non-Physical definitions,
and attaches five externally verified governance references (NIST AI 100-1;
OWASP Top 10 for LLM Applications 2025) to directly matching cards. Every
textual change carries \code{human\_review\_status=pending} and retains the
original text in a \code{definition\_revision} record; no HOLD marker is
cleared by this pass.

The revised card text is then re-audited with the constrained-EM procedure of
Section~3.7, including the destination-specific definition guards: a
candidate destination is rejected unless the card text contains a
destination-anchoring term, which prevents matches driven by generic shared
tokens such as \emph{harm}. An unguarded control run produced 47 proposals,
of which several were exactly the generic-term artifacts the guards exist to
reject (for example, environmental-harm cards attracted to Self-harm); the
guarded run reduces this to 22 defensible proposals that converge in four
iterations. Examples include \emph{Misinformation generation} (Network
Effects to Misinformation/Disinformation, improvement 0.036) and
\emph{Personal information in prompt} (Policy Exposure to Privacy,
improvement 0.024). All 22 movers carry \code{decision\_required=true}; 14
newly enter the review queue and 8 were already held, so the queue grows from
720 to 734. No Physical path changed.

\subsection{Encoder-transfer reliability of the v2.17.0-rc audit}

The sandbox environment used for this release could not obtain the canonical
BGE-M3 weights, so the audit and its reliability analysis were computed with
the independent multilingual encoder \code{intfloat/multilingual-e5-small}
(384 dimensions, seed 20260721, identical procedure and thresholds). This
substitution doubles as the encoder-version experiment requested in
Section~7.6: results below characterize the taxonomy geometry under a second
encoder and are \emph{not} directly comparable to the BGE-M3 values reported
elsewhere in this document. A rerun script
(\code{scripts/run\_v2\_17\_audit\_pipeline.py}) reproduces the full pipeline
with BGE-M3 outside the sandbox.

\begin{table}[ht]
\centering
\caption{Encoder-transfer reliability (multilingual-e5-small) before and
after the guarded v2.17.0-rc audit.}
\label{tab:v217-encoder-transfer}
\begin{tabular}{lrrrr}
\toprule
Metric & All, pre & All, post & Non-HOLD, pre & Non-HOLD, post \\
\midrule
Cards assessed & 1,711 & 1,711 & 991 & 991 \\
Top-1 containment & 60.2\% & 60.3\% & 72.0\% & 72.9\% \\
Top-3 containment & 83.1\% & 83.6\% & 89.5\% & 89.8\% \\
Top-5 containment & 91.4\% & 91.5\% & 93.7\% & 93.7\% \\
Median assignment margin & 0.0022 & 0.0024 & 0.0060 & 0.0061 \\
Negative-margin share & 39.8\% & 38.6\% & 28.0\% & 27.0\% \\
Permutation $p$ & 0.0002 & 0.0002 & 0.0002 & 0.0002 \\
Agreement at $\sigma=0.01$ & 89.7\% & 89.9\% & 91.6\% & 92.0\% \\
Agreement at $\sigma=0.05$ & 52.2\% & 52.5\% & 59.0\% & 59.2\% \\
\bottomrule
\end{tabular}
\end{table}

Three observations carry over from the BGE-M3 analyses. First, the
non-random-structure result replicates: within-family cohesion exceeds all
5,000 matched-size permutations under the second encoder in every condition.
Second, the HOLD separation replicates: the non-HOLD subset is uniformly
stronger (top-1 72.0--72.9\% versus 60.2--60.3\%), confirming that the review
marker concentrates weak-fit cards independently of the encoder. Third,
global reliability remains undemonstrated: absolute containment and
$\sigma=0.05$ stability are lower under e5-small than under BGE-M3, which is
consistent with the reviewer diagnosis that the published partition is
encoder-sensitive and that single-prototype families cannot represent
multi-axial L3 intensions. The guarded audit changes reliability only
marginally (at most $+0.9$ points top-1 on the non-HOLD subset), as expected
for 22 moves in a 1,711-card population; its value is corrective placement of
individually misfiled cards, not a global geometry improvement. The
v2.17.0-rc conclusion therefore remains \textbf{SHARE WITH CAVEATS}, and the
review priorities of Section~8 are unchanged, with the 22 guarded-audit
movers added to the front of the queue after the 39 forced migrations.
