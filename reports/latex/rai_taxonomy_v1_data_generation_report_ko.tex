...
별도의 자동 smoke 12개도 모두 통과했다. 기존 grain·ID·open-set·전문가 합의 검사에 더해 모든 source ID의 결정적 RAI4 대응, Physical 카드별 전체 사슬·근거·3H 보존, 고정 algorithm configuration, 사이트의 exact canonical join과 정적 HTML의 v1.0.0 데이터 경로 계약을 검사했다. 마지막 통합 smoke는 격리 복제본에 후속 릴리스를 실제 작성하고 validator를 한 번 실행해 통과하는지, 미변경 1,506개 결정 ID가 모두 유지되는지를 확인했다. 데이터 품질 노트북은 9개 셀 중 코드 6개를 위에서 아래로 실행했으며 오류 0건, display/image output 0건이었다.
