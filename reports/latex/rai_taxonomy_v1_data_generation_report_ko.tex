...
신규 독립 저장소 \code{RAI-Risk-Taxonomy}에 데이터 registry, placement, crosswalk, LaTeX 검증 보고서 원본, 사이트용 파생 번들과 정적 HTML 탐색기를 생성했다. 기존 \code{AI\_Topic\_Space.github.io} 및 \code{Physical-AI-Risk-Taxonomy} 저장소는 읽기 전용 출처로 취급했으며 변경하지 않았다. 사용자 지시에 따라 컴파일된 PDF는 최종 패키지에 남기지 않았고, GitHub remote 설정, push, Pages 배포도 수행하지 않았다.
...
\midrule
\endhead
\input{generated/source_rows.tex}
\end{longtable}
...
\midrule
\endhead
\input{generated/abstention_rows.tex}
\end{longtable}
...
\midrule
\endhead
\input{generated/cluster_rows.tex}
\end{longtable}
\end{landscape}
...
별도의 자동 smoke 14개도 모두 통과했다. 기존 grain·ID·open-set·전문가 합의 검사에 더해 모든 source ID의 결정적 RAI4 대응, Physical 카드별 전체 사슬·근거·3H 보존, 고정 algorithm configuration, 사이트의 exact canonical join, 정적 HTML의 v1.0.0 데이터 경로 계약, Stage 2 약 90\% 배치와 Stage 3 100\% 강제 매칭의 순차 불변 조건을 검사했다. 후속 릴리스 통합 smoke는 격리 복제본에 릴리스를 실제 작성하고 validator를 한 번 실행해 통과하는지, 미변경 1,506개 결정 ID가 모두 유지되는지를 확인했다. 데이터 품질 노트북은 그래프·이미지 없이 위에서 아래로 실행했다.
...
\code{public/data/releases/v1.0.0/}에는 카드 1,726개, 계층 노드 60개, 검색 인덱스 1,726개가 생성되었다. 각 카드에는 RAI4 ID, 위험명, 현재 상태, 현재 L3 또는 null, source crosswalk가 포함된다. 루트 \code{index.html}과 \code{assets/site.css}, \code{assets/site.js}는 Physical AI 사이트의 헤더·범주 탐색·리스크 카드 패턴을 따르면서 글로벌 규모에 맞춘 검색, L1--L3 필터, 상태 필터, 1,507개 결정 큐, pagination과 카드 상세 dialog를 제공한다. 로컬 브라우저 smoke에서 전체 1,726개 결과, L1 영역 3개, 첫 페이지 카드 36개를 확인했다. GitHub Pages 공개는 수행하지 않았다.
...
\midrule
\endhead
\input{generated/l3_distribution_rows.tex}
\end{longtable}
\end{landscape}
...
\midrule
\endhead
\input{generated/alias_rows.tex}
\end{longtable}
...
\midrule
\endhead
\input{generated/proposed_rows.tex}
\end{longtable}
...
\midrule
\endhead
\input{generated/validation_rows.tex}
\end{longtable}
\normalsize
