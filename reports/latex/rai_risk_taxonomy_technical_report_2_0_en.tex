...
Top-down evidence selection and staged classification of 1,726 global AI risks}
...
risk-card deduplication yield 1,726 L4 risk cards. A staged hierarchy-blind
classification procedure assigns 1,671 cards (96.8\%) to 50 L3 nodes and
withholds 55 cards (3.2\%) in a non-taxonomic review bucket. We specify the
...
AI-risk records, 1,726 canonical L4 cards, and 1,671 L3-assigned cards.
...
independently of the risk label. The result is 1,726 canonical L4 cards. The
...
The final policy view assigns 1,671 of 1,726 L4 cards to one of 50 L3 nodes
(96.8\%). The remaining 55 cards (3.2\%) are stored in \code{RAI-HOLD}, an
operational review queue outside the L0--L3 hierarchy. The hold queue is not an
unapproved 51st L3.
...
\addplot[fill=natureblue, draw=natureblue] coordinates {
  (1671,L3-assigned L4) [1,671]
  (1726,Canonical L4) [1,726]
...
Canonical L4 & 1,726 & Risk card & Source crosswalk; semantic-card deduplication; permanent ID; one canonical mechanism per row. \\
L3 assigned & 1,671 & Risk card & Protected Physical mapping or staged non-Physical placement; exclusion of 55 hard holds. \\
\bottomrule
\end{tabularx}
\end{table}
...
The final policy then removes five hard-hold families from the L3 hierarchy:
23 Physical-scope reviews, 17 expert rejections, 7 expert disagreements,
6 multi-mechanism cases, and 2 low-absolute-fit cases. Their policy L3 becomes
null and their operational bucket becomes \code{RAI-HOLD}. The remaining 118
quota holds stay as forced candidates requiring human review.
...
\end{axis}
\end{tikzpicture}
\caption{\textbf{Disposition of the 173 Stage 2 holds.} The 55 hard holds
(vermillion) are excluded from L3 and placed in \code{RAI-HOLD}; 118
suitability-reserve cards (blue) remain forced candidates.}
\label{fig:holds}
\end{figure}
...
The final policy assigns 1,671 cards (96.8\%) and withholds 55 (3.2\%). Of the
assigned cards, 1,140 are General, 349 are Agentic AI, and 182 are Physical AI.
Their shares of assigned cards are 68.2\%, 20.9\%, and 10.9\%, respectively.
All 50 L3 nodes contain at least one card. Anthropomorphism is the largest L3
with 242 cards (14.5\% of assigned cards); the five largest L3 nodes jointly
contain 41.9\%.
...
\addplot[fill=natureblue, draw=natureblue] coordinates {
 (48,Weaponization) (55,Non-Contestability) (64,Conflict)
 (73,Destabilising Dynamics) (74,Excessive Authority) (78,Network Effects)
...
the twelve L3 nodes containing at least 48 cards. Counts exclude the 55-card
\code{RAI-HOLD} queue.}
\label{fig:l3}
\end{figure}
...
count, the AI-risk view represents 2.1\%, final L3 coverage is 96.8\%, and the
review-hold share is 3.2\%. Exact floating-point ratios remain available in the
...
from 82,971 evidence records to 1,726 L4 cards changes the unit of analysis and
...
The 55 \code{RAI-HOLD} cards should be reviewed first, beginning with the 23
Physical-boundary cases and 24 expert rejection/disagreement cases. The 118
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \small\nolinkurl{public/data/releases/v2.0.0/cards.json}
\end{itemize}
