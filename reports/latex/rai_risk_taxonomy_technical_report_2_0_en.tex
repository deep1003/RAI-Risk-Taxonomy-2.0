...
all 1,660 active cards, including the 189 Physical AI cards. Retired merged
records remain preserved in the 1,711-ID registry but are excluded from
reliability metrics because they are no longer independent active risk cards.
The comparison is deliberately not a comparison between Physical and
non-Physical subsets. Instead, it asks whether the full active release becomes
more stable when the HOLD review queue is excluded from centroid construction
and released-L3 containment.

\begin{table}[ht]
\centering
\caption{HOLD sensitivity at v2.17.2 with Physical AI included
...
\midrule
Cards assessed & 1,660 & 895 \\
Physical AI cards included & 189 & 189 \\
Top-1 containment & 70.2\% & 81.0\% \\
Top-2 containment & 84.0\% & 92.0\% \\
Top-3 containment & 89.9\% & 94.7\% \\
Top-5 containment & 94.6\% & 97.1\% \\
Median assignment margin & 0.0150 & 0.0288 \\
Negative-margin share & 29.8\% & 19.0\% \\
Permutation $p$ & 0.0002 & 0.0002 \\
EM iterations to fixed point & 15 & 14 \\
EM objective at convergence & 0.797 & 0.799 \\
EM agreement with released paths & 25.7\% & 32.4\% \\
Agreement at $\sigma=0.01$ & 94.5\% & 96.0\% \\
Agreement at $\sigma=0.05$ & 73.4\% & 80.4\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../validation/v2.17.2/full_sensitivity_bge_m3/overall_physical_included_hold_sensitivity_3panel.pdf}
\caption{HOLD sensitivity analysis for the full v2.17.2 active release with
Physical AI included. (a) Seed-initialized spherical EM reaches a fixed point
after 15 iterations with HOLD included and after 14 iterations with HOLD
excluded. (b) Released-L3 top-$k$ containment improves across $k=1,2,3,5$
after removing HOLD cards from the evaluated active set. (c) Assignment
stability under Gaussian embedding perturbation is higher on the non-HOLD
subset across the tested noise schedule. Shaded bands indicate 95\% intervals
...
together, while non-random structure holds in both conditions. This is the
strongest available evidence that HOLD functions as a genuine triage
separation rather than an artifact of one encoder or one release. The correct
reading remains conditional: the non-HOLD subset is cleaner because the policy
deliberately concentrates weak-fit and multi-mechanism cases in the queue, so
the comparison validates the separation signal, not the accuracy of every
individual non-HOLD assignment. Machine-readable outputs are stored in
\code{reports/validation/v2.17.2/full\_sensitivity\_bge\_m3/}.
