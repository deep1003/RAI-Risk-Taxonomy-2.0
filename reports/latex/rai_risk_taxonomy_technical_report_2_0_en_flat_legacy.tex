...
overlapping L4 intensions. Release v2.17.0 adds a mechanism-definition
revision pass and BGE-M3 constrained-EM audit; all algorithmic movers remain
under review and 734 cards (42.9\%) are marked \code{HOLD} for human taxonomic
review. Release v2.17.1 then applies a minimal Physical AI transfer rule:
seven directly embodied HOLD cards are moved to existing Physical AI L3 paths,
reducing HOLD to 727 while preserving the protected 182-card Physical source
set unchanged. The final full-release BGE-M3 sensitivity test finds statistically
non-random semantic structure, but does not establish global reliability of the
consolidated 50-family assignment when HOLD remains embedded (69.5\% top-1
containment; 74.0\% perturbation agreement at $\sigma=0.05$ after the audit).
Re-evaluation of the 977 non-HOLD cards yields 79.3\% top-1 containment and
80.0\% perturbation agreement. HOLD exclusion improves EM convergence,
released-L3 top-$k$ containment, and perturbation stability, but this is a
conditional subset result rather than a like-for-like accuracy gain. We specify
every evidence filter, classification rule, and validation limit.
\end{abstract}
...
\subsection{v2.17.0: definition revision pass and BGE-M3 constrained-EM audit}
...
Section~3.7 using the pinned local BGE-M3 snapshot
...
\subsection{HOLD reason grouping and review statistics}
...
\subsection{Candidate L3 expansion analysis for General and Agentic AI}
...
\subsection{Counterfactual reliability simulation for candidate L3 expansion}
...
\subsection{Two-dimensional release-state vector-space diagnostic}
...
\subsection{BGE-M3 sensitivity analysis of the v2.17.0 audit}
...
and the same perturbation grid used in Section~7.6. Table~\ref{tab:v217-bge}
...
\subsection{v2.17.2: final L4 deduplication, redefinition, and BGE-M3 rerun}
...
The final rerun shows improvement after the Claude L4 revision pass and
guarded BGE-M3 remap reflection. For all active cards, top-1 containment rises
from 65.5\% to 70.2\%, the negative-margin share falls from 34.5\% to 29.8\%,
and perturbation agreement at $\sigma=0.05$ rises from 72.0\% to 73.4\%.
For active non-HOLD cards, top-1 containment rises from 77.3\% to 79.7\% and
perturbation agreement at $\sigma=0.05$ rises from 78.8\% to 79.8\%. The
result supports publication as a synchronized review release, but the 144
guarded moves and 765 active HOLD records remain subject to human adjudication.
