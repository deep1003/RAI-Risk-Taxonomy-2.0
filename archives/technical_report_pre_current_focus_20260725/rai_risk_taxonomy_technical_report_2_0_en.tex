...
Top-down evidence selection and staged classification of global AI risks\par}
\vspace{8pt}
{\large RAI Risk Taxonomy Project\par}
\vspace{2pt}
25 July 2026 \,·\, journal edition, release candidate v2.18.0-rc
\end{center}

\begin{abstract}
\noindent
This report gives a reproducible, top-down account of how RAI Risk Taxonomy
2.0 was constructed, classified, and validated. A multi-million-record
evidence space of papers, policy reports, and patents is filtered into a
curated AI-risk evidence view, from which a permanent registry of canonical L4
risk cards is built and assigned to a versioned set of semantic L3 families under
a strict L0\textendash{}L4 hierarchy; the previously validated Physical AI taxonomy is
preserved throughout as a locked gold set. Classification proceeds in stages:
a strict open-set gate that releases only unanimous assignments, a
suitability-ranked relaxation, and a forced-path completion, followed by
independent expert review, guarded constrained-EM audits, and a
registry-preserving consolidation that retires only verbatim duplicate
records while keeping every identifier citable. No algorithmic or forced
placement is treated as ground truth: each remains flagged for human
adjudication through an explicit HOLD review state. Validation combines
permutation tests, centroid-containment analysis, embedding-perturbation
stability, and encoder-transfer checks. The evidence shows that the taxonomy
carries statistically non-random semantic structure, that the HOLD mechanism
effectively concentrates weak-fit and ambiguous cases, and that consolidation
of duplicates measurably tightens the semantic geometry, while global
reliability of the single-parent partition remains under review. We argue
that residual HOLD concentration reflects cross-cutting risks and category
overlap rather than mere misclassification, and we specify every evidence
filter, classification rule, and validation limit needed for replication.
\end{abstract}
...
1,711 registered identifiers, and 1,660 independent active L4 cards assigned
to semantic L3 families. The other 51 identifiers remain citable as merged
provenance records.
...
the registry-preserving consolidation pass, and the HOLD review policy.
Section~\ref{sec:results} reports release composition and reliability validation
through the current release candidate (v2.18.0-rc). Section~\ref{sec:discussion}
interprets the validation evidence and its limits, and Section~\ref{sec:conclusion}
states the release decision and review priorities. Version-by-version engineering
history, detailed review policies, exploratory analyses, and reporting conventions
...
The operational history begins with 1,711 level-validated L4 identifiers.
The original three-stage classifier used 50 L3 nodes and retained
...
The current v2.18.0-rc release keeps all 1,711 identifiers in the registry,
marks 51 as merged provenance records, and assigns all 1,660 independent active
cards to one of 54 semantic L3 families. Of the active cards, 614 retain the
\code{HOLD} review marker. Counts from earlier releases in this report are
historical snapshots and are not current hierarchy totals.
...
\begin{axis}[
  ...
  symbolic y coords={Active L3-mapped L4,Registered L4 IDs,Exact-deduplicated candidates,AI-risk evidence,AI evidence},
  ytick={Active L3-mapped L4,Registered L4 IDs,Exact-deduplicated candidates,AI-risk evidence,AI evidence},
  yticklabels={Active L3-mapped L4,Registered L4 IDs,Exact-deduplicated candidates,AI-risk evidence,AI evidence},
...
\addplot[fill=natureblue, draw=natureblue] coordinates {
  (1660,Active L3-mapped L4) [1,660]
  (1711,Registered L4 IDs) [1,711]
...
\end{axis}
\end{tikzpicture}
\caption{\textbf{Top-down evidence and taxonomy funnel at v2.18.0-rc.}
Counts move from document or patent records to registered risk identifiers and
then to independent active cards. The logarithmic axis preserves visibility
...
Registered L4 IDs & 1,711 & Identifier & Exact normalized-label exclusion against 24 authoritative Physical L3 names; all identifiers remain citable. \\
Active L3-mapped L4 & 1,660 & Independent risk card & Registry-preserving consolidation excludes 51 merged records from active analysis; every active card has a semantic L3 path and 614 carry \code{HOLD}. \\
\bottomrule
\end{tabularx}
\end{table}
...
The current semantic hierarchy uses three canonical L2 categories (Interaction
Safety, System Safety, and Societal Impact) plus a HOLD review overlay, and permanent
...
The cumulative historical audits move three Anthropomorphism cards in v2.9.0, five Goal
...
\code{HOLD} until human adjudication. At the end of the v2.12.0 audit sequence,
the domain counts were 1,165 General, 364 Agentic, and 182 Physical cards; the
review queue contained 689 cards. These are historical release counts.
...
\subsection{Registry-preserving consolidation and label-noise remediation (v2.17.2)}
\label{sec:consolidation}

Release v2.17.2 addresses five classes of L4 label noise identified during
review: overly generic names, exact or near-duplicate names, parenthetical or
compound labels mixing description and scope, domain-topic-like names, and
multi-mechanism labels. Every exact normalized-label cluster was adjudicated
card by card against the mechanism definitions. Records whose label and
mechanism definition are verbatim or same-lineage duplicates (for example,
double-indexed 2021/2022 editions of the same survey, or condensed copies of a
source taxonomy) are retired with a \code{merged\_into} pointer, union of
references, and alias preservation; the identifier registry is invariant, and
no record is deleted. Cards with an articulable subtle difference are
preserved and receive unique mechanism-bearing labels, with the distinction
recorded in provenance; twelve initially merged records were revived under
this preservation-first rule. Multi-mechanism cards receive a primary
mechanism label with the secondary axes stored in a
\code{secondary\_mechanisms} field, consistent with the multi-label extension
discussed in Section~\ref{sec:discussion}. All label and definition changes
carry \code{human\_review\_status=pending} with the original text preserved.
...
Figure~\ref{fig:composition} compares the document-type composition of the
integrated evidence space with that of the AI-risk evidence view. The risk view
is markedly more paper-weighted than the full space, which is dominated by
patents. This asymmetry matters for interpretation: patent language describes
mechanisms and applications rather than harms, so the curated risk view
supplies most of the harm-mechanism vocabulary that the downstream card
registry draws on.
...
At v2.18.0-rc, the registry contains 1,711 identifiers and the hierarchy
coverage calculation uses 1,660 independent active cards. The other 51
identifiers are retained as merged provenance records and are excluded from
active-card statistics. All 1,660 active cards have a semantic L3 assignment;
614 cards (37.0\%) also carry the \code{HOLD} review marker.

The official primary-path counts are 1,220 General-purpose AI cards (73.5\%),
281 Agentic AI cards (16.9\%), and 159 Physical AI cards (9.6\%). The reduction
in the Physical primary-path count reflects the migration of societal-impact
families into the shared General branch, not deletion of source identifiers.
The semantic hierarchy contains 54 L3 families: 33 General, 6 Agentic, and 15
Physical. When HOLD cards are attributed to their retained semantic review
paths, every semantic L3 family is occupied and contains at least one non-HOLD
card. At L2, the corresponding semantic counts are 846 System Safety, 707
Interaction Safety, and 107 Societal Impact cards.

Anthropomorphism is the largest semantic L3 family with 160 cards (9.6\%);
the five largest families jointly contain 553 cards (33.3\%).
...
\addplot[fill=natureblue, draw=natureblue] coordinates {
 (51,Conflict) (53,Destabilising Dynamics) (59,Weaponization)
 (61,Excessive Authority) (65,Network Effects) (81,Privacy)
 (85,Policy Exposure) (88,Non-Contestability) (93,Copyrights)
 (93,Misinformation/Disinformation) (119,Goal Misalignment)
 (160,Anthropomorphism)
};
\end{axis}
\end{tikzpicture}
\caption{\textbf{Largest semantic L3 families at v2.18.0-rc.} The figure
shows the twelve largest families among 1,660 active cards. HOLD cards are
counted under their retained semantic review paths; merged records are
excluded.}
\label{fig:l3}
\end{figure}

\subsection{Historical HOLD composition at v2.17.0}
...
\subsection{Reliability validation procedure}
...
\subsection{Reliability of the v2.17.0 audit}
...
Three conclusions follow from Table~\ref{tab:v217-bge} without restating its
entries. First, the permutation test rejects random labeling under BGE-M3 in
every condition, so the partition encodes real semantic structure rather than
lexical accident. Second, excluding the HOLD queue improves every indicator
simultaneously, which is the signature of an effective triage mechanism: the
review state absorbs the cards whose definitions genuinely underdetermine a
single family. Third, the constrained audit improves local geometry but
cannot, by construction, validate the taxonomy globally: unconstrained
spherical EM lacks the hierarchy, the protected Physical lock, and the review
policy, so its disagreement with released paths measures the difference
between a purely geometric partition and a normative one, not an error rate.
The release conclusion therefore remains 	extbf{SHARE WITH CAVEATS}, with
the guarded reassignment proposals and the HOLD queue as the priority
adjudication targets.

\subsection{Historical release state (v2.17.2)}

Release v2.17.2 incorporates the final L4 label-deduplication and
redefinition pass. The ID registry is invariant at 1,711 registered L4 IDs.
Of these, 1,660 records are displayed and validated as active cards, while 51
records are retained as merged provenance records with \code{merged\_into}
links. The site and report therefore state both numbers together:
\textbf{1,711 registered IDs, 1,660 active cards}. This preserves citation,
audit, and crosswalk stability while preventing exact or same-lineage duplicate
records from being treated as independent active risks.

The BGE-M3 constrained-EM mapping algorithm was rerun on the 1,660 active
cards using the unchanged 50 semantic L3 families from v2.17.1. The 189
Physical-path cards are locked. The rerun produced 144 guarded move
candidates. Following the release policy, these candidates are not treated as
human-approved ground truth: every reflected move remains a HOLD review item.
For cards already in HOLD, only the stored semantic review path is updated; for
previously non-HOLD non-Physical cards, the primary path is moved into the
appropriate General or Agentic HOLD review path and the proposed semantic L3 is
stored in \code{hold\_semantic\_path}. This yields 765 active HOLD records.

\begin{table}[ht]
\centering
\caption{BGE-M3 reliability after the v2.17.2 L4 revision pass.}
\label{tab:v2172-bge-active}
\begin{tabular}{lrrrr}
\toprule
Metric & All, pre & All, post & Non-HOLD, pre & Non-HOLD, post \\
\midrule
...
Table~\ref{tab:v2172-bge-active} shows that the consolidation and guarded
remap pass improves every reliability indicator on both populations. The
mechanism of the gain is informative: retiring verbatim duplicates removes
near-identical points that previously split family mass across identical
intensions, and mechanism-bearing relabeling moves card text closer to the
definitional anchor of its family. The improvement is therefore a data-quality
effect, not an algorithmic recalibration, and it supports publication as a
synchronized review release while every reflected move remains a HOLD review
item pending human adjudication.


\subsection{Historical HOLD sensitivity at v2.17.2}
\label{sec:holdsens-current}

The HOLD sensitivity analysis of Appendix~\ref{app:hist} is repeated here at
the v2.17.2 release state so that the historical triage claim is reproducible.
The procedure applies the BGE-M3 active-card embeddings to
three separately evaluated scopes: the full active release with Physical AI
included and HOLD included, the same full active release after removing HOLD
cards, and the Physical AI subset alone. Retired merged records remain
preserved in the 1,711-ID registry but are excluded from reliability metrics
because they are no longer independent active risk cards.

\begin{table}[ht]
\centering
\caption{Three-scope sensitivity analysis at v2.17.2
(BGE-M3, active cards only, 5{,}000 permutations, 200 perturbation repeats per
$\sigma$).}
\label{tab:holdsens-v2172}
\small
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../validation/v2.17.2/three_scope_sensitivity_bge_m3/overall_hold_included_3panel.pdf}
\caption{Sensitivity analysis for the full v2.17.2 active release with
Physical AI included and HOLD included. (a) Seed-initialized spherical EM
reaches a fixed point after 15 iterations. (b) Released-L3 top-$k$ containment
is evaluated across $k=1,2,3,5$. (c) Assignment stability is measured under
Gaussian embedding perturbation. Shaded bands indicate 95\% intervals across
200 perturbation repeats.}
\label{fig:holdsens-v2172-full}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../validation/v2.17.2/three_scope_sensitivity_bge_m3/overall_hold_excluded_3panel.pdf}
\caption{Sensitivity analysis for the full v2.17.2 active release with
Physical AI included and HOLD excluded. Removing HOLD cards increases Top-1
containment from 70.2\% to 81.0\%, increases Top-5 containment from 94.6\% to
97.1\%, and increases assignment stability at $\sigma=0.05$ from 73.4\% to
80.4\%.}
\label{fig:holdsens-v2172-full-nohold}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../validation/v2.17.2/three_scope_sensitivity_bge_m3/physical_only_3panel.pdf}
\caption{Sensitivity analysis for the Physical AI subset alone. The 189 active
Physical AI cards converge in 6 EM iterations, reach 90.5\% Top-1 containment
and 100.0\% Top-5 containment, and maintain 87.1\% assignment stability at
$\sigma=0.05$.}
\label{fig:holdsens-v2172-physical-only}
\end{figure}

The qualitative pattern of the historical analysis replicates under the
canonical BGE-M3 encoder after the v2.17.2 consolidation pass: removing the
review queue improves containment, margin quality, and perturbation stability
together, while non-random structure holds in both full-release conditions.
The Physical AI subset remains the strongest and most stable reference
population. The correct reading remains conditional: the non-HOLD subset is
cleaner because the policy deliberately concentrates weak-fit and
multi-mechanism cases in the queue, so the comparison validates the separation
signal, not the accuracy of every individual non-HOLD assignment.
Machine-readable outputs are stored in
\code{reports/validation/v2.17.2/three\_scope\_sensitivity\_bge\_m3/}.

\subsection{Current release candidate (v2.18.0-rc)}
\label{sec:v218-current}

Release candidate v2.18.0-rc preserves all 1,711 registered identifiers and
1,660 active cards. It introduces General AI Societal Impact, adds four
semantic L3 families, and migrates the former Physical societal families into
the shared General societal branch. The released semantic hierarchy therefore
contains 54 L3 families: 33 General-purpose AI, 6 Agentic AI, and 15 Physical
AI families. The two HOLD navigation paths remain review overlays and are not
counted as semantic L2 or L3 categories. The revised mapping releases 151
cards from HOLD and leaves 614 active cards pending human review.

The current sensitivity analysis compares all 1,660 active cards with the
1,046-card subset obtained after excluding the 614 HOLD records. Both
conditions use the same BGE-M3 embeddings, 54 L3 seeds, random seed,
permutation design, and 200 Gaussian perturbation repeats. HOLD exclusion
shortens convergence from 20 to 12 iterations. The final mean cosine objective
changes from 0.8005 to 0.8026, Top-1 containment rises from 71.9\% to 81.6\%,
and perturbation stability at $\sigma=0.05$ rises from 75.0\% to 80.7\%.

\begin{table}[H]
\centering
\caption{Current HOLD sensitivity results at v2.18.0-rc.}
\label{tab:holdsens-v218}
\small
\begin{tabular}{lrr}
\toprule
...
Perturbation stability, $\sigma=0.01$ & 95.0\% & 96.0\% \\
Perturbation stability, $\sigma=0.05$ & 75.0\% & 80.7\% \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Current HOLD sensitivity analysis. The figure title omits the release
identifier for reuse; the release state is recorded in this caption and in the
machine-readable validation metadata. The comparison supports HOLD as a
boundary-review signal, not as evidence that every non-HOLD assignment is
correct.}
\label{fig:holdsens-v218}
\end{figure}

\subsection{Historical policy-subset separation at v2.14.0}

At v2.14.0, the HOLD queue was measurably weaker than the non-HOLD subset under
the same released-centroid geometry. This historical result supports the review
marker as a triage instrument, although it does not prove that every individual
marker is correct.
...
The subset ordering in the table matches what the review policy predicts: the
locked Physical set is strongest, the non-HOLD subset intermediate, and the
HOLD queue and forced migrations weakest. Two interpretive cautions apply. The
Physical result evidences internal alignment of the authoritative set but is
not an unbiased comparison, because its paths are locks rather than
predictions. The HOLD result validates the marker as a triage instrument
without proving any individual marker correct, since the policy itself
concentrates weak-fit cases in the queue. Successive releases changed these
subset statistics only marginally, so the release verdict rests on the
qualitative separation rather than on any single decimal.
...
one L3 parent. The \code{secondary\_mechanisms} field introduced in v2.17.2
provides the first data support for this extension while preserving
single-point operational accountability through the mandatory primary parent.

\subsection{Candidate L3 expansion}

A conservative candidate-L3 analysis and its earlier counterfactual reliability
simulation (Appendix~\ref{app:expansion}) initially treated four General and
two Agentic families as review hypotheses. Release candidate v2.18.0-rc adopts
the four General families after a separate extension review: Energy Consumption
and Environmental Pollution, Lack of Accountability and Governance Framework,
Lack of Transparency, and Fairness. The two Agentic candidates remain
exploratory and are not adopted. The earlier counterfactual simulation is
retained as historical evidence and did not itself authorize the release
change.

\section{Conclusion and review priorities}
\label{sec:conclusion}

The release conclusion is \textbf{SHARE WITH CAVEATS}: v2.18.0-rc preserves
1,711 registered identifiers, analyzes 1,660 independent active cards across
54 semantic L3 families, and retains 614 cards for human review. Excluding
these boundary-review cards raises Top-1 containment from 71.9\% to 81.6\%
and perturbation stability at $\sigma=0.05$ from 75.0\% to 80.7\%. The small
objective change from 0.8005 to 0.8026 indicates that the principal gain is
assignment separation rather than a large shift in average semantic proximity.

Review should begin with the 614-card v2.18.0-rc HOLD queue, especially
low-margin cards affected by the new General societal families and by the
Physical-to-General societal migration. Adjudication should require agreement
...
\subsection{Complete English\textendash{}Korean localization (v2.5)}
...
\subsection{Definition audit and Agentic expansion (v2.7\textendash{}v2.8)}
...
\subsection{Historical HOLD baseline (v2.12.0\textendash{}v2.13.0)}
...
\subsection{Full-release results before HOLD separation (v2.12.0\textendash{}v2.14.0)}
...
\subsection{HOLD sensitivity analysis (v2.15.0, historical)}
...
\vspace{6pt}
\noindent\emph{This document is the journal edition of RAI Risk Taxonomy Technical Report 2.0 at release candidate v2.18.0-rc.}
