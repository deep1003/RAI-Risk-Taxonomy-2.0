...
through the current release state (v2.17.2). Section~\ref{sec:discussion}
...
After the v2.17.1 minimal Physical transfer and the v2.17.2 consolidation
(Appendix~\ref{app:history}), the public release comprises 1,711 registered
IDs, 1,660 active cards, 189 Physical-path cards, and 765 active HOLD records.
...
\subsection{Current release state (v2.17.2)}
...
\subsection{HOLD sensitivity at the current release state (v2.17.2)}
\label{sec:holdsens-current}

The HOLD sensitivity analysis of Appendix~\ref{app:hist} is repeated here at
the current release state so that the triage claim is not supported only by a
superseded release. The procedure applies the BGE-M3 active-card embeddings to
...
statistically non-random semantic structure, an effective HOLD triage
separation, and measurable reliability gains from the v2.17.2 consolidation,
but strong global assignment reliability is not yet demonstrated.

Review should begin with the v2.17.2 HOLD queue, especially the 144 guarded
BGE-M3 move candidates, then the weakest global families and low-margin cards.
The 765 active cards marked \code{HOLD} should be adjudicated using agreement
